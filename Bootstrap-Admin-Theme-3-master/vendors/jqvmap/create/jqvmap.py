...
class JQVMap:
    ...
    def getJSCode(self):
        map = {"paths": self.paths, "width": self.width, "height": self.height,
               "insets": self.insets, "projection": self.projection}
        header = "/** JQVMap " + self.projection[
            'type'] + " map for " + self.name + "  */"
        js = "jQuery.fn.vectorMap('addMap', '" + self.name + "'," + json.dumps(
            map) + ");"
        return header + "\n" + js
...
class Converter:
    def __init__(self, config):
        ...
        if args.get('viewport'):
            self.viewport = map(lambda s: float(s),
                                args.get('viewport').split(' '))
        else:
            self.viewport = False
        ...
        projString = '+proj=' + str(
            self.projection) + ' +a=6381372 +b=6381372 +lat_0=0'
    ...
    def convert(self, data_source, output_file):
        ...
        for inset in self.insets:
            ...
            envelope.append(
                shapely.geometry.box(
                    inset['left'], inset['top'], inset['left'] + inset['width'],
        ...
        insetBbox = self.renderMapInset(data_source, main_codes, self.left,
                                        self.top, self.width)
        insetHeight = (insetBbox[3] - insetBbox[1]) * (
                    self.width / (insetBbox[2] - insetBbox[0]))
        envelope.append(
            shapely.geometry.box(self.left, self.top, self.left + self.width,
        ...
        self.map.projection = {"type": self.projection,
        ...
        if self.for_each is not None:
            for code in codes:
                ...
                for param in ('input_file', 'output_file', 'where', 'name'):
                    childConfig[param] = childConfig[param].replace('{{code}}',
                                                                    code.lower())
    ...
    def renderMapInset(self, data_source, codes, left, top, width):
        ...
        for geometry in geometries:
            ...
            if self.simplify_tolerance:
                geom = geom.simplify(self.simplify_tolerance * scale,
                ...
                for ring in rings:
                    for pointIndex in range(len(ring.coords)):
                        ...
                        if pointIndex == 0:
                            ...
                            path += 'l' + str(round(
                                point[0] / scale - ring.coords[pointIndex - 1][
                                    0] / scale, self.precision))
                            path += ',' + str(round(
                                ring.coords[pointIndex - 1][1] / scale - point[
                                    1] / scale, self.precision))
                    path += 'Z'
            self.map.addPath(path, geometry.properties[
                self.config['code_field']].lower(),
...
class DataSource:
    def __init__(self, config):
        ...
        self.spatialRef = osr.SpatialReference()
        projString = '+proj=' + str(
            self.config['projection']) + ' +a=6381372 +b=6381372 +lat_0=0'
    ...
    def create_grammar(self):
        root_table = SymbolTable("root",
                                 map(lambda f: Bind(f['name'], GeometryProperty(
                                     f['name'])), self.fields)
                                 )
            ...
            if geometry.geom is not None:
                ...
                for index, field in enumerate(self.fields):
                    if field['name'] in geometry.properties:
                        feature.SetField(index, geometry.properties[
    ...
    def simplifyRing(self, ring):
        ...
        if not isPivot:
            ...
            if len(simpleRing.coords) <= 2:
                ...
                simpleLineKey = self.format % coords[0] + ':' + self.format % \
                                coords[1] + ':' + self.format % coords[-1]
            ...
            for i in range(1, len(points)):
                ...
                if ((len(self.connections[pointStr]) > 2) or (
                    ...
                    lineKey = self.format % line[-1] + ':' + self.format % line[
                        -2] + ':' + self.format % line[0]
                    if lineKey in self.simplifiedLines:
                        simpleLine = self.simplifiedLines[lineKey]
                        simpleLine = list(reversed(simpleLine))
                    else:
                        simpleLine = shapely.geometry.LineString(line).simplify(
                            self.tolerance).coords
                        lineKey = self.format % line[0] + ':' + self.format % \
                                  line[1] + ':' + self.format % line[-1]
...
class Processor:
    ...
    def union(self, config, data_source):
        ...
        for key in groups:
            geometries.append(
                Geometry(shapely.ops.cascaded_union(groups[key]['geoms']),
    ...
    def merge(self, config, data_source):
        ...
        for rule in config['rules']:
            ...
            geometries = filter(lambda g: expression(g.properties),
                                data_source.geometries)
            geometries = map(lambda g: g.geom, geometries)
            new_geometries.append(
                Geometry(shapely.ops.cascaded_union(geometries),
                         rule['fields']))
    ...
    def remove(self, config, data_source):
        ...
        data_source.geometries = filter(lambda g: not expression(g.properties),
                                        data_source.geometries)

    def remove_fields(self, config, data_source):
        data_source.fields = filter(lambda f: f.name not in config['fields'],
                                    data_source.fields)

    def remove_other_fields(self, config, data_source):
        data_source.fields = filter(lambda f: f['name'] in config['fields'],
                                    data_source.fields)

    def buffer(self, config, data_source):
        for geometry in data_source.geometries:
            geometry.geom = geometry.geom.buffer(config['distance'],
                                                 config['resolution'])

    def simplify_adjancent_polygons(self, config, data_source):
        simple_geometries = PolygonSimplifier(
            map(lambda g: g.geom, data_source.geometries)).simplify()
    ...
    def remove_small_polygons(self, config, data_source):
        for geometry in data_source.geometries:
            if isinstance(geometry.geom,
                          shapely.geometry.multipolygon.MultiPolygon):
                polygons = geometry.geom.geoms
            else:
                polygons = [geometry.geom]
            polygons = filter(lambda p: p.area > config['minimal_area'],
                              polygons)
            if len(polygons) > 0:
